 ...
 *
 * Modified by David Kopec 2/8/26
 ...
 */
int ck_mdb(hfsvol *vol, int options)
{
  MDB *mdb = &vol->mdb;
  time_t now;
  ...
  if (HFSCK_VERBOSE_ENABLED(options))
    {
      int flag;
  ...
  if (mdb->drSigWord != HFS_SIGWORD &&
      ask(options, "Bad volume signature (0x%04x); should be 0x%04x",
  ...
  if (mdb->drCrDate == 0 &&
      ask(options, "Volume creation date is unset"))
    {
  ...
  if (d_ltime(mdb->drCrDate) > now &&
      ask(options, "Volume creation date is in the future (%s)", mctime(mdb->drCrDate)))
    {
      mdb->drCrDate = d_mtime(now);
      vol->flags |= HFS_VOL_UPDATE_MDB;
    }
  ...
  if (mdb->drLsMod == 0 &&
      ask(options, "Volume last modify date is unset"))
    {
  ...
  if (d_ltime(mdb->drLsMod) > now &&
      ask(options, "Volume last modify date is in the future (%s)",
	  mctime(mdb->drLsMod)))
    {
  ...
  if (mdb->drLsMod < mdb->drCrDate &&
      ask(options, "Volume last modify date is before volume creation"))
    {
      mdb->drLsMod = mdb->drCrDate;
      vol->flags |= HFS_VOL_UPDATE_MDB;
    }
  ...
  if (mdb->drVBMSt != 3 &&
      ask(options, "Volume bitmap starts at unusual location (%u), not 3"))
    {
      mdb->drVBMSt = 3;
      vol->flags |= HFS_VOL_UPDATE_MDB;
    }
